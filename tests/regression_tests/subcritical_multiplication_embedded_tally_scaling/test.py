...
class MGXSTestHarness(PyAPITestHarness):
    ...
    def _get_results(self, hash_output=False):
        ...
        with openmc.StatePoint(statepoint) as sp:
            ...
            outstr += 'mg_absorption:\n'

            tally = sp.get_tally(name='mg_absorption')

            mean = tally.mean.ravel()
            std = tally.std_dev.ravel()

            for m, s in zip(mean, std):
                outstr += form.format(m, s)
...
def slab_model():
    ...
    # Multigroup absorption-rate tally
    energy_filter = openmc.EnergyFilter(ebins)

    abs_tally = openmc.Tally(name="mg_absorption")
    abs_tally.filters = [energy_filter]
    abs_tally.scores = ["absorption"]

    model.tallies = openmc.Tallies([abs_tally])
...
def test_slab_modes(slab_model, run_mode, prefix):
    ...
    if run_mode == "fixed source":
        ...
        slab_model.settings.embedded_tally_scaling = True  # Enable embedded tally scaling for subcritical multiplication mode 
